...
const ROUND_MIN_UNLOCK_SECONDS = { 1: 0, 2: 480, 3: 960, 4: 1680, 5: 2400 };
const TIMED_EVIDENCE_RELEASES = [
  { sec: 300, evidence: ['service_corridor_camera_gap'], label: 'Camera review unlocked' },
  { sec: 780, evidence: ['vet_clearance_addendum'], label: 'Vet clearance addendum received' },
  { sec: 1320, evidence: ['tommys_odds_call_note'], label: 'Betting note released' },
  { sec: 2160, evidence: ['preimpact_incapacitation'], label: 'Medical finding released' }
];
const TIMED_TOPIC_RELEASES = {
  marlon_corridor: 660,
  diane_clearance: 840,
  tommy_staff_contact: 1380,
  eddie_timeline_change: 900
};
...
const NAV_ITEMS = [
  ...
  { id: 'storyline', label: 'Storyline', always: true },
...
function checkpointReady(round){
  return checkpointUnlocked(round) && roundTimeUnlocked(round);
}
...
function startTimer(){
  ...
  timerInterval = setInterval(()=>{
    ...
    updateRunUI();
    if(timedChanged || state.currentTab === 'checkpoint' || state.currentTab === 'scene' || state.currentTab === 'storyline' || state.currentTab === 'interviews' || state.currentTab === 'clues'){
      renderNav();
      render();
    }
  }, 1000);
...
function renderHome(){
  return `<div class="start-card"><h2>${escapeHtml(game.case?.title || 'Crime 004')}</h2><div class="body">${escapeHtml(game.case?.setting || '')}\n\nPress Start to begin Attempt 1. Home will hide once the run begins.</div></div>`;
}
...
Attempts
• Start = Attempt 1
• New Attempt restarts the run and increases your attempt count

Timed Progression
• Round 1 opens immediately
• Round 2 checkpoint unlocks at 08:00
• Round 3 checkpoint unlocks at 16:00
• Round 4 checkpoint unlocks at 28:00
• Round 5 checkpoint unlocks at 40:00
• A checkpoint opens only when both the required investigation is complete and the round timer threshold has been reached
• Some clues are released automatically as timed file updates
• Some interview topics unlock only after enough time has passed
• Use the Optional Deduction Panel while waiting for timed releases</div></div>`;
}
...
This case now unfolds over time. As the clock advances, additional phases of the investigation open.</div>
    <div class="footer-actions" style="margin-top:14px">
      <button class="primary" onclick="goToSceneFromStoryline()">Go to Scene</button><div class="progress-meta" style="margin-top:10px">Timed case release: R2 08:00 • R3 16:00 • R4 28:00 • R5 40:00 • timed file updates and interview unlocks continue between rounds</div>
    </div>
  </div>`;
}
...
function submitCheckpointAnswer(sceneId){
  ...
  if(next){
    clearTimeout(checkpointAdvanceTimer);
    checkpointAdvanceTimer = setTimeout(() => { goScene(next); }, CHECKPOINT_ADVANCE_DELAY_MS);
  }
}
